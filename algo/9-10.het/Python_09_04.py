...
class Graph:
    ...
    def rec_DepthFirst(self, start, visited=None):  # MélységiBejárás; rekurzív megvalósítás
        if visited == None:
            # ha a visited halmaz nincs megadva:
            visited = set()
            # akkor legyen üres
        visited.add(start)
        # tegyük bele a halmazba a start pontot
        print(visited)
        # írassuk ki a start pontot ("feldolgozás")

        for tmp in start:
           # iteráljunk végig a start pont szomszédain tmp-vel
            if tmp not in start:
               # ha a tmp segédpont nincs a halmazban:
                  rec_DepthFirst(tmp, visited)
                 # rekurzív hívás átadva a tmp-t és a halmazt
        return
    # visszatérés
